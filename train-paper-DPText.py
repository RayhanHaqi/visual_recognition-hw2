...
# ================= DATA & AUGMENTASI =================
# DPText: Simulasi teks pudar, fotokopi, atau nge-blur
...
class DPText_DETR(nn.Module):
    def __init__(self, version="v1"):
        ...
        # INOVASI DPText
        config.num_queries = 1000          # 1000 Queries
        config.matcher_class_cost = 5.0    # Hukuman berat salah kelas (misal 8 vs 3)
        config.loss_ce = 5.0               # Fokus Loss ke Klasifikasi
...
def main():
    for version in ["v1", "v2"]:
        print(f"\n{'='*50}\n🔎 DPText LOOP | MODEL: {version.upper()}\n{'='*50}")
        bs = INITIAL_BATCH_SIZE; log_file = f"{LOG_DIR}/log_dptext_{version}.csv"
        ...
        model = DPText_DETR(version).to(DEVICE)
        ...
        tr_ds = CustomCocoDetection(DATA_ROOT_TRAIN, ANN_FILE_TRAIN, transform=dptext_transform)
        ...
        while ep < EPOCHS:
            try:
                ...
                if mAP_utama > best_map:
                    best_map = mAP_utama; torch.save(ema.module.state_dict(), f"{CHECKPOINT_DIR}/dptext_{version}_best.pth")
                torch.save(ema.module.state_dict(), f"{CHECKPOINT_DIR}/dptext_{version}_last.pth")
                ep += 1
