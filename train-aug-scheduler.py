...
LOG_FILE = "./logs/training_log_aug_scheduler.csv" 
NUM_CLASSES = 10                     
...
if not os.path.exists(LOG_FILE):
    with open(LOG_FILE, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Epoch", "Train_Loss", "Val_Loss", "mAP_50_95", "mAP_50", "mAP_75", "Learning_Rate"])

# ==========================================
# 2. LIGHTER DATA AUGMENTATION
# ==========================================
train_transform = A.Compose([
    A.Resize(IMG_SIZE, IMG_SIZE),
    # Aman: Merubah kontras, kecerahan, dan warna (kebal foto malam/siang)
    A.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1, p=0.6),
    # Aman: Mensimulasikan kamera buram
    A.OneOf([
        A.GaussianBlur(blur_limit=(3, 5)),
        A.MedianBlur(blur_limit=5),
        A.GaussNoise(var_limit=(10.0, 30.0)),
    ], p=0.4),
    # SHIFTSCALEROTATE DIHAPUS karena merusak bounding box digit!
...
# ==========================================
# 3. DATA PREPARATION (CLIPPING SAFE)
# ==========================================
...
def validate_and_evaluate(model, loader, device, val_json_path):
    ...
    if len(predictions) > 0:
        tmp_json = "tmp_val_preds_adv.json"
...
# ==========================================
# 5. MAIN EXECUTION WITH SCHEDULER
# ==========================================
def main():
    ...
    # 🌟 SENJATA RAHASIA: Cosine Annealing LR
    ...
    while epoch < EPOCHS:
        try:
            ...
            print(f"\n--- Epoch {epoch+1}/{EPOCHS} (Batch Size: {INITIAL_BATCH_SIZE}) | LR: {current_lr:.6f} ---")
            ...
            # Step scheduler setiap akhir epoch
            scheduler.step()
            ...
            with open(LOG_FILE, mode='a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([epoch+1, train_loss, val_loss, map_50_95, map_50, map_75, current_lr])
