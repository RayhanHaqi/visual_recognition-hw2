...
# ⚠️ PERUBAHAN 1: Import class khusus V2 dari Hugging Face
...
# ⚠️ PERUBAHAN 2: File log dipisah agar tidak menimpa log baseline lama
LOG_FILE = "./logs/training_log_v2.csv"
NUM_CLASSES = 10                     
...
# 3. MODEL (RT-DETR V2)
# ==========================================
class RTDETRv2_System(nn.Module):
    def __init__(self, num_classes=10):
        super(RTDETRv2_System, self).__init__()
        
        # ⚠️ PERUBAHAN 3: Memanggil model V2 dari repositori resmi
...
def train_one_epoch(model, loader, optimizer, scaler, device):
    ...
    for images, targets, _, orig_sizes in progress_bar:
        ...
        # Mencegah gradient explosion (sangat penting untuk model baru)
...
def validate_and_evaluate(model, loader, device, val_json_path):
    ...
    if len(predictions) > 0:
        tmp_json = "tmp_val_preds_v2.json"
...
def main():
    ...
    model = RTDETRv2_System(num_classes=NUM_CLASSES).to(DEVICE)
    ...
    while epoch < EPOCHS:
        try:
            ...
            # ⚠️ PERUBAHAN 4: Menyimpan dengan nama V2 agar tidak tertukar
            if map_50_95 > best_map:
                best_map = map_50_95
                torch.save(model.state_dict(), f"{CHECKPOINT_DIR}/rtdetr_v2_r50_best.pth")
                print(f"⭐ Best Model Saved! (New Highest mAP: {best_map:.4f})")
                
            torch.save(model.state_dict(), f"{CHECKPOINT_DIR}/rtdetr_v2_r50_last.pth")
