        ...
        for images, targets, image_ids, orig_sizes in tqdm(loader, desc="Validation", leave=False):
            ...
            for i in range(len(images)):
                ...
                for box, score, label in zip(outputs.pred_boxes[i][keep], scores[keep], labels[keep]):
                    cx, cy, norm_w, norm_h = box.tolist(); w, h = norm_w * orig_w, norm_h * orig_h
...
def main():
    ...
    print(f"📊 Log akan disimpan dalam format CSV: {csv_path}")
    
    # Inisialisasi CSV Header
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Epoch', 'Train_Loss', 'mAP_50_95', 'Learning_Rate'])
    ...
    while epoch <= args.epochs:
        ...
        for i, (images, targets, _, orig_sizes) in pb:
            images = images.to(DEVICE); formatted_targets = prepare_targets(targets, orig_sizes, DEVICE)
            with torch.amp.autocast('cuda'): loss = model(images, labels=formatted_targets).loss / GRAD_ACCUM_STEPS
        ...
        print(f"📈 Ep {epoch} | T_Loss: {avg_train_loss:.4f} | mAP: {mAP:.4f} | LR: {cur_lr:.6f}")
        
        # Simpan ke CSV (Append Mode)
        with open(csv_path, 'a', newline='') as f:
            csv.writer(f).writerow([epoch, round(avg_train_loss, 4), round(mAP, 4), cur_lr])
        ...
        if early_stopper(mAP, epoch): print(f"🛑 [AUTO-EXIT] Early Stopping aktif!"); break
        epoch += 1
